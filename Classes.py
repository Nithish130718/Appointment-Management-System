...
class Doctor:
    ...
    def __init__(self, id=None):
      # registration for new doctor into the system
        if id == None:
            print("\nCreating and Getting Doctor Details:\n")
            self.doctor_id = self.retrieve_last_added_id() + 1
            name = input("Enter your name: ")
            # for inputs like James Jacob
            try:
                self.first_name = name.split(" ")[0]
                self.last_name = name.split(" ")[1]
            # for inputs like Alex (not last name or surname)
            except:
                self.first_name = name
                self.last_name = None
            self.specialization = input("Enter specialization: ")
            col_spl = db["<collection name maintained for specialization data>"] 

            # to add the doctor under respective specialization in the database
            ...
            # if the specialization is not in database -> creates one
            else:
                spl = Specialization()
                spl.insert_data()
            ...
            self.location = {}
            self.location["latitude"], self.location["longitude"] = 15.499493, -83.2829           # Latiutude and longitude of the doctor's clinic
            self.work_schedule = {
                "monday": self.mn_hr,
                "tuesday": self.tue_hr,
            ...
            self.status = False
        # used during retrieval of data about the doctor in further modules
        else:
            self.doctor_id = id
            doctor_document = db["<Doctor collection's name>"].find_one({"doctor_id": self.doctor_id})
            if doctor_document:
    ...
    # retrieval of ID of last enrolled doctor of the database (for enrollment of ID for current doc)
    def retrieve_last_added_id(self):
        col = db["<Doctor collection's name>"]
        doc_list = list(col.find({}))
        # print(doc_list)
        return doc_list[-1]["doctor_id"]

  
    # Function to create time slots based on work hours of the doc
    # The slots are designed as such:  1) each slot is of duration 45 minutes,  2) 15 minute break between each apppointment slot   3) lunch hour fixed between 12 to 1 pm for all doctors (change line 110 for changes in lunch duration, 125 for changes in lunch start time)
    ...
    # function to insert data into the mongodb collection of doctors in the database
    def insert_data(self):
        collection = db["<Doctor collection's name>"]
        data_to_insert = {
    ...
    # To update status of the doctor when a doc checks in / out (live changes in database).
    # Trigger this function from your RFID sensor using a HTTPS protocol 
    # Use 'requests' module & flask to integrate the https protocol request here and make changes.
    """
...
class Patient:    
    ...
    def __init__(self, id=None):
        # registration for new patient into the system
        if id == None:
            ...
            self.location = {}
            self.location["latitude"] = 12.78654        # Latitude and longitude of the kiosk machine.
            self.location["longitude"] = 80.214442
            self.phone_no = input("Enter your number:")
        # used during retrieval of data about the patient in further modules
        else:
            self.patient_id = id
            patient_document = db["<Patient's collection name>"].find_one({"patient_id": self.patient_id})
            if patient_document:
    ...
    # retrieval of ID of last enrolled doctor of the database (for enrollment of ID for current doc)
    def retrieve_last_added_id(self):
        col = db["<Patient's collection name>"]
        doc_list = list(col.find({}))
        # print(doc_list)
    ...
    # Function to insert data about the new patient into the database
    def insert_data(self):
        collection = db["<Patient's collection name>"]
        data_to_insert = {
...
class Specialization:
    def __init__(self, id=None):
        # If specialization exists in database 
        if id == None:
            ...
            special_document = db["<Specialization collection's name>"].find_one({"specialization_id": self.specialization_id})
            if special_document:
    ...
    # retrieval of ID of last enrolled specialization of the database (for enrollment of ID for current speciliazation)
    def retrieve_last_added_id(self):
        col = db["<Specialization collection's name>"]
        doc_list = list(col.find({}))
        # print(doc_list)
    ...
    # Insertion of new specialization collection into the database
    def insert_data(self):
        collection = db["<Specialization collection's name>"]
        data_to_insert = {
...
# --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
                                                      # run this driver code to check if all integrations of this module has been done properly and changes are updated in database.
# --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
